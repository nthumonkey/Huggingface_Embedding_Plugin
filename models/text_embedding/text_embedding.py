...
class HuggingfaceEmbeddingModelsTextEmbeddingModel(TextEmbeddingModel):
    ...
    def _invoke(
        ...
        """
        api_endpoint = credentials.get(
            "api_endpoint", "http://host.docker.internal:8001/embedding/"
        )

        request_data = {"model_name": model, "texts": texts}
        ...
        try:
            ...
            return TextEmbeddingResult(embeddings=embeddings, model=model, usage=usage)
    ...
    def validate_credentials(self, model: str, credentials: dict) -> None:
        ...
        try:
            ...
            if result.get("status") != "ok":
                raise CredentialsValidateFailedError(
                    f"API reports unhealthy status: {result}"
                )

        except requests.exceptions.RequestException as e:
            raise CredentialsValidateFailedError(
                f"Could not connect to embedding service: {str(e)}"
            )
